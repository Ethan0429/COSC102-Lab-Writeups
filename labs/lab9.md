...
- `string` - This might seem obvious, you'll have to use it in the lab anyway of course. But remember that a `string` is BASICALLY just a `vector` but all of its elements are `char`s. It gives provides the same methods your familiar with from `vector` like `push_back()` and `pop_back()` - two things I used in my code.
